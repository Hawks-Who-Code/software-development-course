...
All code in a Git repository is stored on a computer called the *origin* server which is normally run by a third party company such as GitHub. Once the server has a repository, any developer given access to the repository can *clone* (copy) it to the local drive of a computer. Once copied to a computer, developers can change code without immediately affecting the code that everyone else has access to. This allows multiple people to work on different features at the same time without other code changing around them.
